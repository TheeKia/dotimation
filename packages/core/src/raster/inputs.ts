...
/**
 * Everything that affects rasterization output. The runtime scheduler re-runs the
...
export interface RasterInputs {
  ...
  dprEpoch: number
  /** Bumped when the item's custom web font finishes loading (see runtime/environment.ts). */
  fontEpoch: number
}
