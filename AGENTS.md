...
- `bun run release <version> --dry-run` — preview release prerequisites and reviewed notes without mutations. Omit `--dry-run` to validate, bump all three published packages, commit, annotate and atomically push the release tag with `main`. Notes default to `docs/releases/<version>.md`; `--notes <file>` imports an external Markdown file. See `docs/releases/README.md` for prerequisites and recovery. Stable releases use npm `latest`; prereleases use `next`.
