...
import { DEFAULT_MOTION, toSimParams } from '@/engine/params'
...
declare global {
  interface Window {
    lifecycle: {
      render(next: Partial<Props>): void
      ...
      unmount(): void
      stats: { backend: string; particles: number } | null
    }
  }
}
  ...
  async verifyGpu(kind) {
    ...
    const backend =
      kind === 'webgl2'
        ...
        : (await import('@/backends/webgpu')).createWebGPUBackend(params)
