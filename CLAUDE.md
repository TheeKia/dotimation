...
- **Unit tests** live under `bun test` and cover the pure cores (field, sampler, simulate, render, settle, clock, prng, raster-inputs). DOM/React-bound code (rasterize, engine, backends, component) is exercised manually through the `test/ui` playground.
