...
3. Push a `v<version>` tag to trigger the release workflow. It verifies that the tag matches all package versions and reruns browser/packed-consumer checks.
4. The workflow publishes core first, then React and Svelte with public access, and generates the changelog. A failed partial release can be retried; `--tolerate-republish` skips versions already published from that tag. For manual publication, use the same order and flags from each package directory.

The new scoped package names require access to the `@dotimation` registry scope before the first release. This refactor does not publish packages or change the existing release version. Every package's prepublish hook rebuilds and validates output; package tests use `--ignore-scripts` while packing already-built artifacts to avoid recursive validation.
