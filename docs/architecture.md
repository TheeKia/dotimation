...
Bun is pinned in `.bun-version`. CI runs on pull requests and pushes to `main`; tags run only the release workflow. Builds, distribution checks and unit tests run on all three operating systems; lint, types, playgrounds and browser tests run on Linux. Browser failures retain screenshots and Playwright traces for seven days. Release runs are serialized without cancellation, and release-note failures fail the publishing job so they can be retried.
