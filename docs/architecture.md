...
4. The workflow builds and packs once on Linux, tests those archives, and passes them to a separate publishing job. That job publishes core first, then React and Svelte with public access, and generates the changelog. A failed partial release can be retried: `scripts/publish-packages.ts` checks the public registry and skips versions already published. Registry and publishing errors fail the job. For manual publication with Bun, use the same package order and `--access public --tolerate-republish` from each package directory.
